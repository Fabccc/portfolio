...
#[component]
pub fn StateView(#[prop(into)] store: Store<State>) -> impl IntoView {
    view! {
        <div class="w-full flex flex-row justify-around">
            <span
                class=move || tailwind_classname(store.memory_usage().get(), MAX_MEMORY_USAGE)
            >{move || format!("{:.1} Gi", store.memory_usage().get())}</span>
            <span>"/"</span>
            <span>"196Gi"</span>
        </div>
        <div class="w-full flex flex-row justify-around">
            <span
                class=move || tailwind_classname(store.cpu_usage().get(), MAX_CPU_USAGE)
            >{move || format!("{:.1} cpu", store.cpu_usage().get())}</span>
            <span>"/"</span>
            <span>"48 cpu"</span>
        </div>
        <div class="w-full flex flex-row justify-around">
            <span
                class=move || tailwind_classname(store.pods_count().get() as f32, MAX_PODS_COUNT as f32)
            >{move || format!("{} pods", store.pods_count().get())}</span>
            <span>"/"</span>
            <span>"160pods"</span>
        </div>
    }
